...
class Rescaler:
    def __init__(self, dataset_type_train, dataset_type_validate, raw_data_dir=os.path.join(os.getcwd(), "data")):
        self.summand, self.mult = self.compute_renorms(raw_data_dir, dataset_type_train, dataset_type_validate)
    def compute_renorms(self, raw_data_dir, dataset_type_train, dataset_type_validate):
        ...
        dataset_norms_la = (np.mean(X_la, axis = (0,2))[None, None, :, None], np.std(X_la, axis = (0,2))[None, None, :, None]) # single feature
        dataset_norms_bay = (np.mean(X_bay, axis = (0,2))[None, None, :, None], np.std(X_bay, axis = (0,2))[None, None, :, None])
    ...
    with torch.no_grad():
        for s, data in enumerate(test_loader):
            ...
            if s % (len(test_loader) // 100 + 1) == 0:
                print(f"\r {s}/{len(test_loader)}, MSE: {metrics.compute()['MSE'].mean()}", end="")
