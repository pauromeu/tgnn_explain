...
def main():
    ...
    maximum = np.max(node_masks[np.arange(len(node_masks)) != node])
    for shape in shape_types:
        mask = np.where(point_shapes == shape)[0]
        if len(mask) > 0:
            plt.scatter(coordinates[mask, 1], coordinates[mask, 0], s=sizes[mask], c=node_masks[mask], cmap="OrRd", marker=shape, vmin=0.05, vmax=0.7)

    # take top and right axes off
